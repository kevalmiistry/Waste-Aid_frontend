...
const Profile = () => {
  ...
  const { userInfo, fetchUserInfo, fetchTokensFunc, tokens, LogOutFunc } =
    context;
  ...
  const showProfile = () => {
    if (mobileMenu === 'CLOSED') {
      document.querySelector('.profile__main').style.transform =
        'translateY(0%)';
  ...
  return (
    localStorage.getItem('Waste_Aid_authtoken') && (
      <>
        <img
          onClick={showProfile}
          className="profile_img_mobile"
          src={profileImage}
          alt="WasteAid User profile"
        />
